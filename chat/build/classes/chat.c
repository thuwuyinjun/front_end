...
JNIEXPORT jint JNICALL Java_chat_TTS(JNIEnv *env, jclass jc, jstring text, jstring name)
{
	const char *text_str = (*env)->GetStringUTFChars(env, text, 0);
	const char* filename             = (*env)->GetStringUTFChars(env, name, 0); //合成的语音文件名称
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = 57051125, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
	...
	*/
	...
	/* 文本合成 */
	printf("开始合成 ...\n");
	ret = text_to_speech(text_str, filename, session_begin_params);
	if (MSP_SUCCESS != ret)
	{
